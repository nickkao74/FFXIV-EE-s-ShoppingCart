...
export async function handleApi(request: Request, url: URL, env: Env): Promise<Response> {
  ...
  if (path === '/api/auth/discord' && request.method === 'GET') {
    ...
    headers.set('Location', `https://discord.com/api/oauth2/authorize?${params.toString()}`);
    headers.append('Set-Cookie', makeOAuthStateCookie(state));
  ...
  if (path === '/api/logout' && request.method === 'POST') {
    const headers = new Headers();
    headers.append('Set-Cookie', makeExpiredSessionCookie());
    return jsonResponse({ ok: true }, 200, { 'Set-Cookie': makeExpiredSessionCookie() });
  }
  ...
  if (path === '/api/orders' && request.method === 'POST') {
    ...
    const selfSupply = body.selfSupply;
  ...
  if (path === '/api/stock' && request.method === 'PATCH') {
    if (session.role !== 'admin') return forbiddenError('沒有管理權限');
    const body = await parseJson(request);
    const stock = await patchStock(env, body.stock || {});
    return jsonResponse({ ok: true, stock });
  }
  ...
  if (path === '/api/stock' && request.method === 'PUT') {
    if (session.role !== 'admin') return forbiddenError('沒有管理權限');
    const body = await parseJson(request);
    const stock = await putStock(env, body.stock || {});
    return jsonResponse({ ok: true, stock });
  }
