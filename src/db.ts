...
interface D1EquipmentHistoryRow {
  lines_json: string;
  status: string;
  created_at: number;
  completed_at: number | null;
...
export async function getEquipmentHistoryForUser(
  env: Env,
  discordUserId: string
): Promise<EquipmentHistoryOrder[]> {
  const result = await env.DB
    ...
    `)
    .bind(discordUserId)
    .all<D1EquipmentHistoryRow>();

  const orders: EquipmentHistoryOrder[] = [];
